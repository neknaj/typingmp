...
フリックキーボードで「い」または「え」を入力した際、目標が ゐ・ゑ であれば正解と判定される。直接かな一致 (Path 1) では `normalize_typing_char` が ゐ/ゑ を い/え として正規化し、ローマ字入力 (Path 2) でも `layout_data.rs` のエイリアスにより i/e 入力を許容する。
