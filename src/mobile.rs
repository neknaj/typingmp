    ...
    for item in render_list {
        match item {
            ...
            | Renderable::Text { text, anchor, shift, align, font_size, color } => {
                ...
                gui_renderer::draw_text(pixel_buf, width, font, &text, (x as f32, y as f32), pfs, color);
            }
            Renderable::TypingUpper { segments, anchor, shift, align, font_size } => {
                let pfs = calculate_pixel_font_size(font_size, width, height);
                let ruby_pfs = pfs * 0.4;
                let total_width: u32 = segments.iter()
                    .map(|s| gui_renderer::measure_text(font, &s.base_text, pfs).0)
                    .sum();
                let total_height = gui_renderer::measure_text(font, " ", pfs).1;
                let ap = ui::calculate_anchor_position(anchor, shift, width, height);
                let (mut pen_x, y) = ui::calculate_aligned_position(ap, total_width, total_height, align);
                for seg in segments {
                    ...
                    gui_renderer::draw_text(pixel_buf, width, font, &seg.base_text, (pen_x as f32, y as f32), pfs, color);
                    ...
                    match seg {
                        ...
                        LowerTypingSegment::Active { elements } => {
                            for el in elements {
                                ...
                                gui_renderer::draw_text(pixel_buf, width, font, &text, (pen_x as f32, y as f32), pfs, color);
            ...
            Renderable::ProgressBar { anchor, shift, width_ratio, height_ratio, progress, bg_color, fg_color } => {
                ...
                gui_renderer::draw_rect(pixel_buf, width, sx, sy, bw as usize, bh as usize, bg_color);
